...
public class SentenceMerger {
     ...
     * 本方法写入清理标记让已排队的旧 roundId claim 失败，避免 unbind 后继续触发 AI。
     */
    public void cleanup(String callId) {
        ...
        try {
            redisTemplate.opsForValue().set(stateKey(callId), CLEANED_ROUND_MARKER,
                    STATE_TTL_BUFFER_MS, TimeUnit.MILLISECONDS);
            log.debug("[M02] Cleanup marker set, callId={}", callId);
        } catch (DataAccessException e) {
            log.warn("[M02] Redis cleanup marker failed, callId={}", callId, e);
        }
    }
